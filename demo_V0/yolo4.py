...
class MobileNetV3(nn.Module):
    ...
    def forward(self, x):
        # the first [52,52,40] needs 7=1+1+2+3 conv
        out3 = self.model.features[:7](x)
...
#---------------------------------------------------#
#   三次卷积块
#   DarknetConv2D_BN_Leaky卷积
####### 用3*3深度可分离卷积替换conv3*3  5*5里面的3*3卷积
#---------------------------------------------------#
def make_three_conv(filters_list, in_filters):
    m = nn.Sequential(

        # conv2d(filter_in, filter_out, kernel_size, groups=1, stride=1):
        conv2d(in_filters, filters_list[0], 1),

        ### origin::  conv2d(filters_list[0], filters_list[1], 3),
...
#---------------------------------------------------#
#   最后获得yolov4的输出
# conv 3*3( abstract features)+ conv 1*1(for result)
#---------------------------------------------------#
def yolo_head(filters_list, in_filters):
    m = nn.Sequential(

        ### origin::: conv2d(in_filters, filters_list[0], 3),
...
#  def __init__(self, in_channels, out_channels):
        ...
        # filters: 512->256     map size: *2

        # print shape =
        # for (26,26,512) ->(26,26,256)???
        self.conv_for_P4     = conv2d(in_filters[1], 256,1)
        # def make_five_conv(filters_list, in_filters):
        # out::  filters_list[0] --->256
...
# 256->128
    ...
    def forward(self, x):
        ...
        # PANet: the first below concat  # 1：按维数1拼接 [2,3]+[2,4]=[2,7]
        # (26,26,256)+
...
# #  def __init__(self, in_channels, out_channels):
        ...
        #P3: one part down sampling ,the other part goto yolohead
        ...
        out2 = self.yolo_head3(P3)   #upper one
        out1 = self.yolo_head2(P4)
        out0 = self.yolo_head1(P5)  #down
